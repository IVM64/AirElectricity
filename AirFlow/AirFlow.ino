...
void initDashboard(){
  ...
  drawtext(0,65,1,"m/s  : 23.4334",SSD1327_WHITE);
...
//Initialize the Sparkfun Air Velocity Sensor FS3000
bool initAirFlowSensor(){
...
//Read the air velocity
void readAirFlow(airData *airRecord){
  
    airRecord->raw = fs.readRaw(); // note, this returns an int from 0-3686
    airRecord->mtspersec = fs.readMetersPerSecond(); // note, this returns a float from 0-7.23
    airRecord->mph = fs.readMilesPerHour(); // note, this returns a float from 0-16.17
     ...
     startTime = millis();
      ...
      if (req != NULL) {
          ...
          if (body != NULL) {
              ...
              sprintf(buf,"%.3f",airDataRecord.mtspersec);
          ...
          if ((millis() - startTimeMQTT > delayBetweenSamplesMQTT)){
            ...
            if (req != NULL) {
              ...
              if (bodyMQTT != NULL) {
                  //Air sensor data
                  J *airMQTT = JCreateObject();
                  JAddNumberToObject(airMQTT, "raw", airDataRecord.raw);
                  JAddNumberToObject(airMQTT, "mtspersec", airDataRecord.mtspersec);
                  JAddNumberToObject(airMQTT, "mph", airDataRecord.mph);
                  JAddItemToObject(bodyMQTT, "air", airMQTT);
                  //Battery monitor data
